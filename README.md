...
`roc_desk-http` 是 roc_desk 多仓库拆分后的独立工具仓库。

## 用途

用于创建、管理和执行 HTTP 请求，支持集合、环境变量、请求历史以及 Postman/OpenAPI 导入导出。

## 依赖

- 公共基础库：`roc_desk-common` 的 `roc_desk_core`，固定使用对应 tag。
- 工具专属依赖：roc_desk-common core；reqwest；serde/serde_json/serde_yaml；uuid；chrono。
...
默认生成 Release 版本：`bin\roc_desk-http.exe`。开发构建使用：
...
```markdown
![HTTP 测试工作台主界面](docs/screenshots/main.png)
```
